...
```
сгенерированный секрет прописывается в файле ==.env== как переменная окружения (откуда он будет потом подставляться в config/secrets.yml):
```
SECRET_KEY=секрет
```
#### 10.
...
Можно экспортировать в systemd:
```
